...
with graph.as_default():
    train_images = placeholder("train_images")
    train_labels = placeholder("train_lables")
    model = VGG(data.n_classes, MIDDLE_SHAPE)
    ...
    with sess.as_default():
        ...
        try:
            for i in xrange(ITERATIONS):
                ...
                feed_dict = {train_images: train_data,
                             train_labels: train_lab_vec,}
            ...
            util.pkl_dump(encoder, os.path.join(workspace.model_dir, "encoder.pkl"))
            if EMAILING is True:
                send_mail(subj, msg)
